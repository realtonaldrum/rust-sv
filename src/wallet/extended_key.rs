...
impl ExtendedKey {
    /// Returns the version bytes based on network and key type
    ...
    pub fn is_private(&self) -> bool {
        ...
        version == MAINNET_PRIVATE || version == TESTNET_PRIVATE
    }
    ...
    pub fn decode(s: &str) -> Result<ExtendedKey> {
        let v = s.from_base58().map_err(|e| Error::FromBase58Error(e))?; // Fixed <Vec<u8>>::from_base58(s)
        if v.len() != 82 {
    ...
    pub fn derive_child(&self, index: u32, secp: &Secp256k1<secp256k1::All>) -> Result<ExtendedKey> {
        ...
        let mut hmac = hmac::Context::new(hmac::Key::new(hmac::HMAC_SHA512, &self.chain_code()));

        if is_private && is_hardened {
            // Hardened derivation for private key
            hmac.update(&[0]);
            hmac.update(&self.key()[1..33]);
        } else if is_private {
            // Normal derivation for private key
            let pubkey = PublicKey::from_secret_key(secp, &SecretKey::from_slice(&self.key()[1..33])?);
            hmac.update(&pubkey.serialize());
        } else {
            // Derivation for public key (only normal)
            if is_hardened {
        ...
        child_key.0[0..4].copy_from_slice(&self.version()); // Same version
        child_key.0[4] = self.depth().wrapping_add(1);
        let parent_fingerprint = sha256d(&self.key()[1..33])[0..4].to_vec();
        child_key.0[5..9].copy_from_slice(&parent_fingerprint);
        child_key.0[9..13].copy_from_slice(&index_bytes);
        child_key.0[13..45].copy_from_slice(&result.as_ref()[32..64]); // Chain code
...
pub fn extended_key_from_seed(seed: &[u8], network: Network) -> Result<ExtendedKey> {
    ...
    let mut hmac = hmac::Context::new(hmac::Key::new(hmac::HMAC_SHA512, b"Bitcoin seed"));
    hmac.update(seed);
    let result = hmac.sign();
    ...
    let version = match network {
        ...
        Network::Testnet | Network::STN => TESTNET_PRIVATE,
    };
...
mod tests {
    ...
    fn test_path() -> Result<()> {
        ...
        let secp = Secp256k1::new();
        let child = master.derive_child(0x80000000, &secp)?; // m/0H

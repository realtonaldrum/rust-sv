...
impl Address {
    ...
    pub fn new(bytes: Vec<u8>, addr_type: AddressType, network: Network) -> Self {
    ...
    pub fn decode(input: &str, network: Network) -> Result<Self> {
...
mod tests {
    ...
    fn test_invalid_addr_type() {
        ...
        let result = addr_decode(&invalid_addr, Network::Testnet);
        assert!(result.is_err());
        assert!(matches!(result, Err(Error::BadData(_))));
    }
}
